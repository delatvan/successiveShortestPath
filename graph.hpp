...
//vector with vertices with balance_ >0
vector<vertex *> pos_balance;
//vector with vertices with balance_ <0
vector<vertex *> neg_balance;
//Path from s to t
vector<edge *> path;
...
//create path to r from the intital s once shortest path alg was run
// and return minimal capacity in path
int create_path(vertex * r);
//augment along path
void augment_path(int gamma);
